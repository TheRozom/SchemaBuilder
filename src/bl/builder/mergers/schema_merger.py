...
class SchemaMerger:
    ...
    def merge(self, first_schema: SchemaNode, second_schema: SchemaNode) -> SchemaNode:
        ...
        for second_option in second_options:
            ...
            for index, first_option in enumerate(first_options):
                first_type = first_option.type if isinstance(first_option, SchemaNode) else first_option.get(SchemaKeyword.TYPE)
                second_type = second_option.type if isinstance(second_option, SchemaNode) else second_option.get(SchemaKeyword.TYPE)

                if first_type == second_type or {first_type, second_type} == {SchemaType.INTEGER, SchemaType.NUMBER}:
        ...
        if len(first_options) == 1:
            return first_options[0] if isinstance(first_options[0], SchemaNode) else SchemaNode(**first_options[0])
        return SchemaNode(anyOf=first_options)

    def _merge_same_type(self, first_schema: Union[SchemaNode, Dict[str, Any]], second_schema: Union[SchemaNode, Dict[str, Any]]) -> SchemaNode:
        first_node = first_schema if isinstance(first_schema, SchemaNode) else SchemaNode(**first_schema)
        second_node = second_schema if isinstance(second_schema, SchemaNode) else SchemaNode(**second_schema)

        first_type = first_node.type
        second_type = second_node.type

        if first_type != second_type and {first_type, second_type} == {SchemaType.INTEGER, SchemaType.NUMBER}:
    ...
    def _merge_integer(self, first_schema: SchemaNode, second_schema: SchemaNode) -> SchemaNode:
        ...
        return SchemaNode(type=SchemaType.INTEGER, minimum=0, maximum=max(first_maximum, second_maximum))
    ...
    def _merge_object(self, first_schema: SchemaNode, second_schema: SchemaNode) -> SchemaNode:
        ...
        for property_key in all_property_keys:
            if property_key in first_properties and property_key in second_properties:
                first_property = first_properties[property_key] if isinstance(first_properties[property_key], SchemaNode) else SchemaNode(**first_properties[property_key])
                second_property = second_properties[property_key] if isinstance(second_properties[property_key], SchemaNode) else SchemaNode(**second_properties[property_key])
    ...
    def _merge_array(self, first_schema: SchemaNode, second_schema: SchemaNode) -> SchemaNode:
        ...
        first_items_node = first_items if isinstance(first_items, SchemaNode) else SchemaNode(**first_items) if first_items else SchemaNode()
        second_items_node = second_items if isinstance(second_items, SchemaNode) else SchemaNode(**second_items) if second_items else SchemaNode()
