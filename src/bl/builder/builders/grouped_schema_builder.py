...
class GroupedSchemaBuilder:
    ...
    async def build_schema(self, data_list: List[Any]) -> Tuple[Dict[str, Any], Optional[AnalysisResult]]:
        if not data_list:
    ...
    async def _build_merged_schema(self, data_list: List[Any]) -> Dict[str, Any]:
        ...
        for item in data_list:
            ...
            if merged_schema is None:
                ...
                first_schema_node = merged_schema if isinstance(merged_schema, SchemaNode) else SchemaNode(**merged_schema)
                second_schema_node = item_schema if isinstance(item_schema, SchemaNode) else SchemaNode(**item_schema)
    ...
    async def _build_anyof_schema(self, data_list: List[Any], analysis: AnalysisResult) -> Dict[str, Any]:
        group_schemas = []
