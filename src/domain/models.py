...
class ValidationResult(BaseModel):
    valid: bool = Field(
        ..., description="Whether all data validates against the schema"
    )
...
class ConflictAnalysis(BaseModel):
    ...
    unique_structures: int = Field(
        ..., description="Number of unique structural groups found"
    )
    should_split_schemas: bool = Field(
        ..., description="Whether schemas should be split"
    )
...
class SchemaDefinition(BaseModel):
    ...
    score: Optional[ScoreResult] = Field(
        None, description="Quality score of the schema"
    )
